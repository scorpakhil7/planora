  ...
  return (
    <div className="flex gap-3 py-3 border-b border-gray-50 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        {activity.notes && <p className="text-xs text-gray-500 mt-0.5">{activity.notes}</p>}
      </div>
      ...
      {open && (
        <div className="px-5 pb-5 space-y-2">
          {day.activities.length > 0 && (
            <div>
              {day.activities.map((activity, i) => <ActivityRow key={i} activity={activity} />)}
            </div>
          )}
...
export default function TripDetailPage() {
  ...
  async function handleGenerateItinerary() {
    ...
    setError("");
    try {
      await post("/ai/itinerary", { trip_id: trip.id, destinations: trip.destinations, start_date: trip.start_date, end_date: trip.end_date });
      await fetchTrip();
    } catch (err: any) {
  ...
  if (loading) return <div className="text-center py-20 text-gray-400">Loading trip...</div>;
  if (error && !trip) return <div className="text-center py-20"><p className="text-red-500">{error}</p><Link href="/trips" className="text-indigo-600 underline mt-4 block">Back to trips</Link></div>;
  if (!trip) return null;
      ...
      {trip.itinerary ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleGenerateItinerary} disabled={aiLoading} className="px-4 py-2 text-sm font-semibold border border-indigo-200 text-indigo-600 rounded-xl hover:bg-indigo-50 disabled:opacity-50 transition-colors">
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-white rounded-2xl border border-indigo-100 p-10 text-center space-y-4">
          ...
          <p className="text-gray-500 text-sm max-w-sm mx-auto">Planora AI will build a detailed day-by-day plan with transport, activities, meals, accommodation and budget — all tailored to your trip.</p>
          <button onClick={handleGenerateItinerary} disabled={aiLoading} className="mt-2 px-8 py-3 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 disabled:opacity-60 transition-colors shadow-sm">
            {aiLoading ? "Generating your itinerary..." : "✨ Generate itinerary"}
          </button>
          {aiLoading && <p className="text-xs text-gray-400 animate-pulse">This usually takes 10-15 seconds...</p>}
        </div>
      )}
    </div>
  );
